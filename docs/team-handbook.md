...
**一个阿里云百炼（DashScope）key 全搞定**：百炼同时托管 Qwen 系（主 LLM /
生图 / VLM / embedding / ASR）和 DeepSeek-V4 系（审计员,2026-04 上架,与
官网同价:Flash 输入 ¥1/百万、输出 ¥2）。一条成品帖全流程 <¥0.5。

| 平台 | 用途 | 备注 |
| --- | --- | --- |
| **阿里云百炼**（唯一必须） | Qwen 写手 + DeepSeek 审计员 + 生图/VLM | 一个 key,OpenAI 兼容口 |
| DeepInfra（美国生产线备用） | 服务美国客户时同一批开源模型的美国托管 | 数据不出美国,见下方私有化一节 |
| Plausible（可选） | 真实网站分析 | 免费层够用,可自托管 |

注意：模型具体型号更新很快（本手册写于 2026-07），配置前用这条命令列出
你账号可见的确切型号名：

```bash
curl -s -H "Authorization: Bearer $KEY" \
  https://dashscope.aliyuncs.com/compatible-mode/v1/models | python -m json.tool | grep '"id"'
```

### .env 完整配置（一个百炼 key 照抄）

```dotenv
LLM_PROVIDER=dashscope
DASHSCOPE_API_KEY=<百炼key>
DASHSCOPE_MODEL=qwen3.6-flash    # 以上面命令列出的实际型号为准

# 审计员:DeepSeek 走百炼的 OpenAI 兼容口(同一个 key,不用注册 SiliconFlow)
SILICONFLOW_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1
SILICONFLOW_API_KEY=<同一个百炼key>
SILICONFLOW_MODEL=deepseek-v4-flash   # 同上,以实际型号名为准

ASSET_DRAFT_FANOUT=3             # 多稿自选
TREND_SOURCE=hackernews          # 美国机器可开;国内服务器连不上 HN,留 mock
...
**审计员自动生效**：只要 `SILICONFLOW_API_KEY` 已配置,seed 会自动把
Content auditor 钉到 siliconflow provider（重部署重 seed 也不会丢）。
已有数据库不想重 seed 时,在 Team tab → Content auditor → provider 手动改
一次即可。验证:Team tab 里该员工的 provider 显示 `siliconflow`。
